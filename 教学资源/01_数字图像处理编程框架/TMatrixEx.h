...
private:
	...
	// [ ****************** ] ............................................
public:
	CTMatrixEx()                                    // [ 构造函数: 缺省构造函数 ]
	{
	    Initialization(); // [ 初始化成员变量 ]
    }

	CTMatrixEx( long rows, long columns )           // [ 构造函数: 分配矩阵内存 ]
	{
	    Initialization(); // [ 初始化成员变量 ]
	...
	CTMatrixEx( const CTMatrixEx< TYPE >& matrix )  // [ 复制构造函数 ]
	{
	    Initialization(); // [ 初始化成员变量 ]
...
public:
	...
	void Construction( const CTMatrixEx< TYPE >& matrix ) // [ 构造矩阵内容: 矩阵复制 ]
	{
		if( matrix.m_memory != NULL ) // [ 判断矩阵内容是否为空 ]
		{
			Construction( matrix.m_rows, matrix.m_columns ); // [ 构造相同尺寸的矩阵: 分配矩阵内存 ]

			long elements = m_rows * m_columns; // [ 矩阵元素个数 ]

			for( int index = 0; index < elements; index++ ) // [ 逐个复制矩阵元素 ]
			{
				m_memory[ index ] = matrix.m_memory[ index ]; // [ 复制矩阵元素 ]
			}
		}
		else
		{
	...
	CTMatrixEx< TYPE >& operator=( const CTMatrixEx< TYPE >& matrix )  // [ 赋值操作符 ]
	{
	    Construction( matrix ); // [ 构造矩阵内容: 矩阵复制 ]
...
#ifdef _DEBUG
	CTArrayEx< TYPE >& operator[]( long index ) const                  // [ 下标操作符 ]
	{
    	static CTArrayEx< TYPE > error; // [ 出错返回的静态变量 ]
		...
		static CTArrayEx< TYPE > rowArray; // [ 矩阵行数组 ]
